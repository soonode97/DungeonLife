...
class Unit {
  ...
  attack(target, logs) {
    ...
    target.unitType !== 'user'
      ? console.log(chalk.bgGreen(attackMessage))
      : console.log(chalk.bgRed(attackMessage));
      console.log(chalk.bgBlack(`[${++logs.length}] ${target.name}의 HP가 ${target.hp}만큼 남았습니다.`));
  }
}

class Player extends Unit {
  constructor() {
    super('Violeto', 50, 10, 10, 'user', 80);
  ...
  escape(logs) {
    ...
    if (isEscape > this.escapeChanceRatio) {
      console.log(chalk.bgGreen(`[${++logs.length}] ${this.name}이 도망치지 못했습니다!!`));
      return false;
    }
    console.log(chalk.bgGreen(`[${++logs.length}] ${this.name}이 도망에 성공했습니다!!`));
    return true;
  }
  ...
  heal(stage = 0, skillRatio = 0, logs) {
    ...
    if (stage !== 0) {
      ...
      console.log(chalk.bgYellow(`${this.name}이 스킬로 인해 ${healAmount}만큼 HP를 회복했습니다.`));
    }
    this.hp += healAmount;
  }
  ...
  async action(player, monster, logs) {
    ...
    console.log(chalk.green(`[${++logs.length}] ${choice}를 선택하셨습니다.\n`));
...
class Stage {
  ...
  displayStatus(player, monster, turn) {
  ...
  checkStage(player, monster) {
    ...
    if (monster.hp <= 0) {
      console.log(chalk.bgGreen(`[${++this.logs.length}] ${monster.name}이 쓰러졌습니다.`));
      player.heal(this.currentStage, 0, this.logs);
